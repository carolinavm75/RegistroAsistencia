...
export default function GestionPeriodosPage() {
  ...
  const periodosFiltrados = useMemo(() => {
    ...
    return periodos
      .filter((periodo) => {
        if (verSoloActivos) return periodo.estado;
        return true;
      })
      .filter((periodo) => {
        ...
        return (
          normalizarTexto(periodo.id).includes(texto) ||
          normalizarTexto(periodo.fecha_inicio).includes(texto) ||
          normalizarTexto(periodo.fecha_fin).includes(texto)
        );
      })
      .sort((a, b) => {
  ...
  }, [periodos, busqueda, verSoloActivos]);

  const totalActivos = periodos.filter((periodo) => periodo.estado).length;
  const totalInactivos = periodos.filter((periodo) => !periodo.estado).length;
  ...
  async function cargarPeriodos() {
    ...
    if (error) {
      setError(`No fue posible cargar periodos: ${error.message}`);
      return;
    }
  ...
  async function guardarPeriodo(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    ...
    if (!fechaInicio || !fechaFin) {
      setError('Selecciona la fecha de inicio y la fecha final.');
      return;
    }
    ...
    if (editandoId) {
      ...
      if (error) {
        setError(`No fue posible actualizar el periodo: ${error.message}`);
        return;
      }
    ...
    if (error) {
      if (error.code === '23505') {
        setError('Ya existe un periodo con ese nombre.');
      } else {
        setError(`No fue posible crear el periodo: ${error.message}`);
      }
  ...
  async function cambiarEstadoPeriodo(periodo: Periodo) {
    setError('');
    setMensaje('');

    const nuevoEstado = !periodo.estado;
    ...
    setMensaje(
      nuevoEstado
        ...
        : 'Periodo inactivado correctamente.'
    );
    ...
    <main className="page">
      <section className="shell">
        ...
        <section className="content-card">
          <div className="section-header">
            <p className="section-label">
              {editandoId ? 'Editar periodo' : 'Nuevo periodo'}
            </p>
          ...
          <form className="form" onSubmit={guardarPeriodo}>
        ...
        <section className="content-card">
          ...
          {cargando && <p className="empty-text">Cargando periodos...</p>}

          {!cargando && periodosFiltrados.length === 0 && (
            <p className="empty-text">No hay periodos para mostrar.</p>
          )}

          <div className="compact-list">
            {periodosFiltrados.map((periodo) => (
              <article key={periodo.id} className="compact-row">
                ...
                <span className={periodo.estado ? 'status active' : 'status inactive'}>
                ...
                <div className="actions">
                  ...
                  <button
                    className={periodo.estado ? 'danger-button' : 'activate-button'}
                    onClick={() => cambiarEstadoPeriodo(periodo)}
                  >
                    {periodo.estado ? 'Inactivar' : 'Activar'}
                  </button>
                </div>
              </article>
            ))}
